...
# these unitests test the functions in data_value_check.py
...
class TestDataValueCheck(TestCase):
    ...
    def test_validate_data_valid_date(self):
        expected = True
        result = validate_data("date_of_birth", "1990-01-01", "date")
        self.assertEqual(expected, result)

    def test_validate_data_invalid_date(self):
        expected = ValueError
        with self.assertRaises(expected):
            validate_data("date_of_birth", "1990-13-01", "date")

    @patch('data_value_check.connection')
    def test_check_invalid_date_ranges(self, mock_connection):
        ...
        mock_cursor.execute.assert_called_with(
            "SELECT id, full_name, date_of_birth FROM PEP_Identification.pep_individuals WHERE pep_individuals.date_of_birth < '1900-01-01' OR date_of_birth < CURDATE()")
    ...
    def test_check_null_values(self, mock_connection):
        ...
        mock_cursor.execute.assert_called_with("SELECT COUNT(*) FROM startups WHERE name IS NULL")
    ...
    def test_insert_pep(self, mock_connection):
        ...
        mock_cursor.execute.assert_called_with(
            "INSERT INTO PEP_Identification.pep_individuals (full_name, job_title, country, date_of_birth) VALUES (%s, %s, %s, %s)",
    ...
    def test_check_eu_countries_blacklist(self, mock_connection):
        ...
        mock_cursor.execute.assert_called_with("Select * FROM eu_countries_blacklist WHERE country_name = %s", ("Russia",))
